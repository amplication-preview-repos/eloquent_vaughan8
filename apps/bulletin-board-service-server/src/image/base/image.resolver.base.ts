...
import { Bulletin } from "../../bulletin/base/Bulletin";
...
export class ImageResolverBase {
  ...
  async createImage(@graphql.Args() args: CreateImageArgs): Promise<Image> {
    return await this.service.createImage({
      ...
      data: {
        ...
        bulletin: args.data.bulletin
          ? {
              connect: args.data.bulletin,
            }
          : undefined,
      },
    });
  ...
  @graphql.ResolveField(() => Bulletin, {
    nullable: true,
    name: "bulletin",
  })
  async getBulletin(@graphql.Parent() parent: Image): Promise<Bulletin | null> {
    const result = await this.service.getBulletin(parent.id);
